...
<script>
  export default {
    ...
    props: {
      ...
      animate: {
        validator: value => {
          return ['bounce', 'fade', 'slide-down', 'slide-left', 'slide-right', 'slide-up', 'zoom-in', 'zoom-out', false]
            .indexOf(value) !== -1
        },
        default: false
      }
    ...
    methods: {
      ...
      closeByBackdrop(e) {
        let overlay = this.$refs.overlay,
          target = e.target;
        if (overlay instanceof Element && this.closeByBackdropClick
          && !(overlay === target || overlay.contains(target))) {
          this.close();
        }
      },
      escapeKey(e) {
