          ...
          <splitpanes class="default-theme" vertical style="height: 60vh" vertical>
            <pane class="p-2" min-size="20" max-size="80" style="background: ghostwhite">
              <div>
                <b-row>
                  <b-col md="12">
                    <b-dropdown text="Select AVA" class="btn btn-md mr-1">
                      <b-dropdown-item @click="openAvaDialog()">AVA List</b-dropdown-item>
                      <b-dropdown-item @click="openAvaZDialog()">AVA List Z</b-dropdown-item>
                    </b-dropdown>
...
<!--                    <b-btn class="btn btn-md mr-1" variant="success" @click="openAvaDialog()">AVA List</b-btn>-->
    ...
    <vue-form-dialog
      ...
      btn1_text="Close" btn2_text="Process"
      btn1_style="danger" btn2_style="primary"
      @btn1Click="avaZDialogBtn1Click()" @btn2Click="avaZDialogBtn2Click()">

      <!-- body slot -->
      <span slot="slot-body" style="padding-left: 20px; padding-right: 20px; width: 100%">
              <vue-form-generator :schema="schema_sgy_pick_z" :model="model_sgy_pick_z" :options="formOptions"
                                  @validated="onValidated"/>
            </span>
    </vue-form-dialog>

    <vue-form-dialog
      ref="probabilityIxzDialog"
      type="default"
      header="Parameters" body="Body"
      btn1_text="Close" btn2_text="Process"
      btn1_style="danger" btn2_style="primary"
      @btn1Click="probabilityIxzDialogBtn1Click()" @btn2Click="probabilityIxzDialogBtn2Click()">
...
<script>
  ...
  export default {
    name: "ProcessWizardStep3_3_WithILZ",
    ...
    data() {
      return {
        ...
        event_http_list_ava_z: {success: "successListAvaZ", fail: "failListAvaZ"},
    ...
    methods: {
      ...
      //------------------------- ava dialog Z ----------------------------
      openAvaZDialog() {
        this.$refs.avaZDialog.showModal();
      },
      avaZDialogBtn1Click() {
        this.$refs.avaZDialog.hideModal();
      },
      avaZDialogBtn2Click() {
        if (!this.bvalidate) return;
        if (this.model_sgy_pick_z["segy_file_id"] === "")
          return;

        this.proc_completed = false;
        this.showLoader = true;
        let param = {
          iline: this.model_sgy_pick_z["iline"],
          xline: this.model_sgy_pick_z["xline"],
          cdp_z: this.model_sgy_pick_z["cdp_z"],
          segy_file_id: this.model_sgy_pick_z["segy_file_id"],
          radius: this.model_sgy_pick_z["radius"],
        };
        this.$store.dispatch('http_post', ["/api/point/find-ava-data", param, this.event_http_list_ava_z]).then();
      ...
      //------------------------- probability IXZ dialog ----------------------------
      openProbabilityILZDialog()
      {
        this.$refs.probabilityIxzDialog.showModal();
      },
      probabilityIxzDialogBtn1Click() {
        this.$refs.probabilityIxzDialog.hideModal();
      },
      probabilityIxzDialogBtn2Click() {
        if (!this.bvalidate) return;
        ...
        this.textIXZ = "[[8063,2441,576.992], [8055,2449,882.132]]";
        let json_obj = JSON.parse(this.textIXZ);
        let tmp_table_datas = [];
        for(let i=0; i<json_obj.length; i++)
        {
          ...
          tmp_table_datas.push(xx);
        }

        let param = {
          mlpick_file_id: this.model["file_id"],
      ...
      probabilityDialogBtn2Click() {
        ...
        for (let i = 0; i < this.table_datas.length; i++) {
          if (this.table_datas[i]["check"] === true) {
            let tmp1 = this.table_datas[i];
            tmp1["check"] = 1;
            tmp_table_datas.push(tmp1)
          }
        }

        if (tmp_table_datas.length <= 0) {
          this.retStatus = {status: 0, title: "Information", message: "Please Select data from table ...", data: []};
          this.$refs.dialogMessage.showModal();
          return;
        }
    ...
    mounted() {
      ...
      EventBus.$on(this.event_http_list_sgy.success, (msg) => {
        ...
        this.schema_sgy_pick_z.fields[3].values = this.list_segy_pick;
        this.getMLPickFile();
      });
      ...
      //-------------- LIST AVA Z-------------------
      EventBus.$on(this.event_http_list_ava_z.success, (msg) => {
        this.table_datas = msg["gather"];
        this.ndata = this.table_datas.length;
        this.showLoader = false;
        this.ava_with_z = true;
      });
      EventBus.$on(this.event_http_list_ava_z.fail, (msg) => {
        this.showLoader = false;
        this.table_datas = [];
        this.retStatus = msg;
        this.$refs.dialogMessage.showModal();
    ...
    beforeDestroy() {
      ...
      EventBus.$off(this.event_http_list_ava_z.success);
      EventBus.$off(this.event_http_list_ava_z.fail);
